...
- 사용자에게 횟수와 자동차 이름을 입력받는다
  - 각 횟수마다 이름을 가진 여러대의 자동차는 전진 또는 정지한다
  - 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException` 을 발생시키고 에러메시지를 출력한다
  
- 각 자동차가 전진하는 조건은 무작위 값이 4이상인 경우이다
- 우승자를 출력하는데, 우승자는 한 명 이상일 수 있다
## 🔍 진행방식
